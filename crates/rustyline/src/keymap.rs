...
impl<'b> InputState<'b> {
    ...
    fn emacs<R: RawReader>(
        ...
        if let Some(cmd) = self.custom_binding(wrt, &evt, n, positive) {
            // PATCHED: Don't call redo() on custom bindings - trust what ConditionalEventHandler returns
            return Ok(cmd);
...
impl<'b> InputState<'b> {
    ...
    fn custom_seq_binding<R: RawReader>(
        ...
        while let Some(subtrie) = self.custom_bindings.get_raw_descendant(evt) {
            ...
            let handler = match subtrie.get(evt) {
                Ok(handler) => handler,
                Err(_) => break, // Key sequence not found, exit loop
            };
            if let Some(handler) = handler {
