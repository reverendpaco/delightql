...
impl SqlValue {
    ...
    /// Convert from duckdb Value
    pub fn from_duckdb_value(row: &duckdb::Row, index: usize) -> duckdb::Result<Self> {
        use duckdb::types::ValueRef;

        match row.get_ref(index)? {
            ValueRef::Null => Ok(SqlValue::Null),
            // DuckDB has many integer types - convert all to i64
            ValueRef::Boolean(b) => Ok(SqlValue::Integer(if b { 1 } else { 0 })),
            ValueRef::TinyInt(i) => Ok(SqlValue::Integer(i as i64)),
            ValueRef::SmallInt(i) => Ok(SqlValue::Integer(i as i64)),
            ValueRef::Int(i) => Ok(SqlValue::Integer(i as i64)),
            ValueRef::BigInt(i) => Ok(SqlValue::Integer(i)),
            ValueRef::HugeInt(i) => Ok(SqlValue::Integer(i as i64)), // May truncate
            ValueRef::UTinyInt(i) => Ok(SqlValue::Integer(i as i64)),
            ValueRef::USmallInt(i) => Ok(SqlValue::Integer(i as i64)),
            ValueRef::UInt(i) => Ok(SqlValue::Integer(i as i64)),
            ValueRef::UBigInt(i) => Ok(SqlValue::Integer(i as i64)),
            // Float types
            ValueRef::Float(f) => Ok(SqlValue::Real(f as f64)),
            ValueRef::Double(f) => Ok(SqlValue::Real(f)),
            // Text types
            ...
            _ => Ok(SqlValue::Text(format!("{:?}", row.get_ref(index)?))),
        }
    }
}
...
impl TypedQueryResult {
    ...
    pub fn to_string_result(&self) -> super::QueryResult {
        let string_rows = self
            .rows
            .iter()
