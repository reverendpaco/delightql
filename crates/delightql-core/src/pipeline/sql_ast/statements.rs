...
impl Cte {
    pub fn new(scope: crate::names::ScopeId, query: QueryExpression) -> Self {
        Cte {
            scope,
            query,
            is_recursive: false,
    ...
    pub fn new_recursive(scope: crate::names::ScopeId, query: QueryExpression) -> Self {
        Cte {
            scope,
            query,
            is_recursive: true,
    ...
    /// The same binding over a rewritten body.
    ///
    /// A pass that rewrites a CTE's query must come back through here. The
    /// alternative — rebuilding with `Cte::new` — silently answers a
    /// question it was never asked, because `new` means non-recursive and
    /// the recursion decision was taken long before this pass ran.
    pub fn with_query(self, query: QueryExpression) -> Self {
        Cte { query, ..self }
    }
}
