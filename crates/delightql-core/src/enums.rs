...
pub enum EntityType {
    ...
    /// Facts are named data literals: person(0, "Gusti", "Parlor")
    /// Multiple clauses with the same name are merged into a single relation
...
pub enum ConnectionType {
    ...
    /// Pipe-based coprocess connection (osqueryi, sqlite3, etc.)
    Pipe = 6,
}
