...
INSERT INTO identifier (kind, hierarchy, summary, explanation) VALUES
    ...
    ('error', 'semantic/ho', 'A higher-order view was parameterized wrongly.', 'Family for refusals of higher-order (parameterized) view forms: scalar/table params, mixed-ground positions, and parameter/column hygiene. HO params bind by AST substitution at the call — a param name is a value spliced into the body, never a column of it. Members: param_shadows_column (a scalar param name collides with a body column), unbound-mixed-param (a mixed-ground position given a bare identifier).'),
    ...
    ('error', 'semantic/ho/unbound-mixed-param', 'A mixed-ground HO position was given a bare identifier.', 'A position that is a ground constant in some clauses and a free scalar in others (MixedGround) demands a concrete value at the call — a literal or an expression, not a bare identifier — so the clause-pruning dispatch can select the matching clauses. Supply a ground value, or bind the identifier from an outer context.'),
    ...
    ('danger', 'cardinality/nulljoin', 'NULL-matching join equality (NULL = NULL → true).', 'DECLARED, NOT YET WIRED (2026-07-17, R-1): join equality is SQL equality, where NULL never matches — and that safe behavior is today''s ONLY behavior; turning this gate ON does not yet change code generation. When wired, ON will make NULLs match each other in join keys, which can multiply rows AND changes what the join means — semantic-class, so inline-only ((~~danger://cardinality/nulljoin ON~~)), never a CLI flag. Consult sys.danger(*) for this session''s states.'),
