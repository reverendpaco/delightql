...
//! one per `fatboy://duckdb/<path>` connection and reaps it on drop; the
//! pipe is the lifecycle. The database is opened READ-ONLY, so multiple
//! fatboy children can share one file across processes (DuckDB's write
//! lock is exclusive); the fatboy is query-only, so no loss.
...
struct Args {
    ...
    /// The fatboy spawn contract is just `--database D`.
...
fn serve_stdio(database: &str) {
    let stdin = std::io::stdin();
    let stdout = std::io::stdout();
    let mut reader = stdin.lock();
    let mut writer = stdout.lock();

    let mut party = match DuckParty::connect(database) {
        Ok(p) => p,
        Err(e) => {
            let mut buf = Vec::new();
    ...
    loop {
        ...
        let response = match msg {
            ...
            ClientMessage::Control(delightql_protocol::ControlOp::Reset) => {
                match DuckParty::connect(database) {
                    Ok(fresh) => {
                        party = fresh;
